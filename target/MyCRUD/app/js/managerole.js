    ...
    return {
        init: function () {
                ...
                "aoColumnDefs":[
                    ...
                    },
                    {
                        ...
                        "mRender": function (data, type, full) {
                            return "<a class=\"edit\" href=\"/rest/page/editrole\">" + "编辑" + "</a>"; //(7) 设置“编辑”超链接
                        }
                    },
                    {
                        "aTargets": [5],
                    ...
                    },
                    {
                        "aTargets": [6],
                        "mRender": function (data, type, full) {
                            return "<a class=\"btn btn-info\" >" + "绑定权限" + "</a>";
                        }
                    }
                ],
                // m410,特别要求
            ...
            function fnRefresh() {
                ...
                $('#tbu a.btn').on('click' ,function (e) {
                    e.preventDefault();
                    var nRow = $(this).parents('tr')[0];          // 获得当前选中“编辑”的行号
                    var aData = oTable.fnGetData(nRow);           // 根据行号获取行数据
                    var url="/rest/page/setrole";
                    $('#EditDiv').load(url,{number1:aData[0].toString(),number2:aData[1].toString()}); // 将行数据中第一个数据，也就是“编号”作为参数number1进行传递
                    $('ListDiv').hide();                                 // EditDiv和ListDiv都在页面manageuser.jsp中
                })
            ...
            function retrieveData(sSource2, aoData2, fnCallback2) {
                ...
                $.ajax({
                    url: "rest/page/dataroletableajax",
